...
import { isNativeAvailable, isWasmAvailable, loadWasmCrypto, isWasmLoaded } from './index.js';
...
describe('account types + constants', () => {
  it('has correct transaction version constants', () => {
    ...
    expect(TRANSACTION_VERSION.V3_QUERY).toBe(0x100000000000000000000000000000003n);
  });
  ...
  it('has correct transaction hash prefixes', () => {
    ...
    expect(TRANSACTION_HASH_PREFIX.DEPLOY_ACCOUNT).toBe(0x6465706c6f795f6163636f756e74n);
  });
});
...
describe('transaction hashes', () => {
  ...
  it('computes deploy account hash', () => {
    ...
    const address = computeContractAddress(tx.class_hash, tx.contract_address_salt, tx.constructor_calldata);
...
describe('selector + address helpers', () => {
