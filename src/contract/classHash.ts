...
import {
  ...
  decodeShortString as decodeShortStringPrimitive,
} from '../primitives/index.js';
import { type Result, ok, err, abiError, type Abi } from '../abi/types.js';
...
// Re-export short string functions from primitives
// These return bigint, we provide hex wrappers for class hash computation
...
export function decodeShortString(felt: bigint | string | Felt252Type): string {
  return decodeShortStringPrimitive(felt);
}
...
function hashEntryPointsSierra(entryPoints: SierraEntryPoint[]): bigint {
  if (entryPoints.length === 0) {
    return toBigInt(safePoseidonHashMany([]));
  }
  ...
  return toBigInt(safePoseidonHashMany(flat));
}
...
function hashBuiltins(builtins: string[]): bigint {
  ...
  const encoded: bigint[] = builtins.map((b) =>
    BigInt(encodeShortString(b))
  );

  return toBigInt(safePoseidonHashMany(encoded));
}
...
function hashSierraProgram(program: string[]): bigint {
  ...
  return toBigInt(safePoseidonHashMany(felts));
}
...
function hashByteCodeSegments(bytecode: string[], segmentLengths: number[]): bigint {
  ...
  const hashLeaves: bigint[] = segmentLengths.flatMap((len) => {
    ...
    segmentStart += len;
    return [BigInt(len), toBigInt(safePoseidonHashMany(segment))];
  });

  return 1n + toBigInt(safePoseidonHashMany(hashLeaves));
}
...
function hashBytecode(casm: CompiledSierraCasm): bigint {
  ...
  return toBigInt(safePoseidonHashMany(casm.bytecode.map((b) => BigInt(b))));
}
...
export function compiledClassHashFromCasm(casm: CompiledSierraCasm): Result<string> {
  try {
    ...
    return ok(toHex(compiledClassHash));
  } catch (error) {
    return err(
      abiError(
        'ENCODE_ERROR',
