...
describe('Short String Encoding', () => {
  it('should encode ASCII string to hex', () => {
    // Vector from starknet.js
    const result = encodeShortString('hello');
    expect(result).toBe('0x68656c6c6f'); // 'hello' in hex
  });

  it('should encode CONTRACT_CLASS_V0.1.0', () => {
    const result = encodeShortString('CONTRACT_CLASS_V0.1.0');
    // This is used in class hash computation
    expect(BigInt(result)).toBeGreaterThan(0n);
  });

  it('should encode COMPILED_CLASS_V1', () => {
    const result = encodeShortString('COMPILED_CLASS_V1');
    expect(BigInt(result)).toBeGreaterThan(0n);
  });

  it('should throw on non-ASCII', () => {
    expect(() => encodeShortString('héllo')).toThrow();
  });

  it('should throw on string > 31 chars', () => {
    const longString = 'a'.repeat(32);
    expect(() => encodeShortString(longString)).toThrow();
  });

  it('should decode hex to ASCII string', () => {
    expect(decodeShortString('0x68656c6c6f')).toBe('hello');
    expect(decodeShortString(0x68656c6c6fn)).toBe('hello');
  });

  it('should roundtrip encode/decode', () => {
    const testStrings = ['hello', 'test', 'CONTRACT_CLASS_V0.1.0', 'COMPILED_CLASS_V1', 'a', ''];
...
describe('classHashFromSierra', () => {
  ...
  const minimalSierra: CompiledSierra = {
    ...
    entry_points_by_type: {
      EXTERNAL: [
        { selector: '0x1234', function_idx: 0 },
      ],
      L1_HANDLER: [],
      CONSTRUCTOR: [],
    },
  ...
  it('should error on missing entry_points_by_type', () => {
    const invalid = { ...minimalSierra, entry_points_by_type: undefined } as any;
    const result = classHashFromSierra(invalid);
    expect(result.error).not.toBeNull();
    expect(result.error!.code).toBe('INVALID_ABI');
  });
...
describe('compiledClassHashFromCasm', () => {
  /**
...
describe('extractAbi', () => {
  it('should extract ABI from Sierra artifact', () => {
    const sierra: CompiledSierra = {
      ...
      abi: [
        { type: 'function', name: 'test', inputs: [], outputs: [], state_mutability: 'view' },
      ],
    };
...
describe('Starknet.js v6.23.1 Golden Vectors', () => {
  ...
  it('documents expected class hash format', () => {
    ...
    const expectedSierraHash = '0x345df0a9b35ce05d03772ba7938acad66921c5c39c1a5af74aee72aa25c363e';
    expect(expectedSierraHash).toMatch(/^0x[0-9a-f]{1,64}$/);

    // Expected compiled class hash format (1-64 hex chars after 0x)
    const expectedCompiledHash = '0x5c82c98f2ab111bd50293ba64bb18cf49037374783ad2486c712709c4ba0d89';
  ...
  it('verifies short string encoding matches starknet.js', () => {
    ...
    const encoded = encodeShortString('CONTRACT_CLASS_V0.1.0');
  ...
  it('verifies COMPILED_CLASS_V1 encoding', () => {
    const encoded = encodeShortString('COMPILED_CLASS_V1');
    expect(BigInt(encoded)).toBe(
