...
Kundera provides primitive event decoding in `@kundera-sn/kundera-ts/abi`. For receipt-wide decoding and filtering, use the event-filtering skill.
...
```typescript
import { decodeEvent, getEventSelector } from '@kundera-sn/kundera-ts/abi';
...
```typescript
import { computeSelector } from '@kundera-sn/kundera-ts/abi';
import { decodeEvents } from './skills/event-filtering';
...
<CardGroup cols={2}>
  <Card title="ABI API" icon="code" href="/typescript/api/abi">
