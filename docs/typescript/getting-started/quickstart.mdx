...
Kundera uses pure JS crypto by default - no setup needed:

```typescript
import { Pedersen, Poseidon, snKeccak } from '@kundera-sn/kundera-ts/crypto';

const hash = Pedersen.hash(a, b);  // Just works
```

<Note>
For performance-critical applications, Native FFI and WASM backends are available. See [Runtime Implementations](/typescript/getting-started/runtime-implementations) for details.
</Note>
