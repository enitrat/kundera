...
const { result, error } = await contract.read('balance_of', [ContractAddress('0x123...')]);
//     ^? Uint256Type (inferred from ABI)
```

## How Type Inference Works

The magic comes from Kundera's `InferReturn`, `InferArgs`, and `InferFunctionName` utility types:

```ts
import type { InferReturn, InferArgs, InferFunctionName } from '@kundera-sn/kundera-ts/abi';

// Given an ABI and function name, extract types:
type BalanceArgs = InferArgs<typeof ERC20_ABI, 'balance_of'>;
//   ^? [ContractAddressType]

type BalanceRet = InferReturn<typeof ERC20_ABI, 'balance_of'>;
//   ^? Uint256Type

type FnNames = InferFunctionName<typeof ERC20_ABI>;
//   ^? 'balance_of' | 'transfer'
```
...
interface Contract<TAbi extends StarknetAbi> {
  read<TFn extends InferFunctionName<TAbi> & string>(
    fn: TFn,
    args: InferArgs<TAbi, TFn>
  ): Promise<ContractResult<InferReturn<TAbi, TFn>>>;

  // Add write with account
  write<TFn extends InferFunctionName<TAbi> & string>(
    fn: TFn,
    args: InferArgs<TAbi, TFn>,
    account: Account
