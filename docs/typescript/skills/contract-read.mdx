...
A minimal read-only contract helper built on `starknet_call`. These calls don't modify state and don't require an account.
...
2. **Call** - `starknet_call` is sent to the provider
...
import { starknet_call } from '@kundera-sn/kundera-ts/rpc';

const { result: calldata } = encodeCalldata(ERC20_ABI, 'balance_of', [accountAddress]);
const rawResult = await starknet_call(transport, {
...
Read methods return decoded values as an array:
...
<CardGroup cols={2}>
  ...
  <Card title="ABI Encode/Decode" icon="code" href="/guides/abi/encode-decode">
