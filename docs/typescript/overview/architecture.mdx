...
The foundation is a set of branded types that represent Starknet values. A `Felt252` is a `bigint` at runtime, but TypeScript's type system prevents you from accidentally passing a `ClassHash` where a `ContractAddress` is expected. The brand is phantom — it costs nothing at runtime.

```typescript
import { Felt252, ContractAddress, ClassHash } from '@kundera-sn/kundera-ts';

const felt = Felt252("0x123");           // bigint & { __tag: "Felt" }
const addr = ContractAddress("0xabc");   // bigint & { __tag: "ContractAddress" }
