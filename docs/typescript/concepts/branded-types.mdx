...
For bundle-size optimization, Kundera also provides a [tree-shakeable functional API](/typescript/concepts/tree-shakeable-api) with the same method signatures but imported as standalone functions.

## All Branded Types

| Type | Description | Max Value |
|------|-------------|-----------|
| [Felt252Type](/typescript/api/primitives#felt252) | Raw 252-bit field element | < 2^251 + 17*2^192 + 1 |
| [ContractAddressType](/typescript/api/primitives#contractaddress) | Starknet contract address | < 2^251 |
| [ClassHashType](/typescript/api/primitives#classhash) | Contract class identifier | < 2^251 + 17*2^192 + 1 |
| [StorageKeyType](/typescript/api/primitives#storagekey) | Storage slot address | < 2^251 + 17*2^192 + 1 |
| [EthAddressType](/typescript/api/primitives#ethaddress) | L1 Ethereum address | < 2^160 |
...
<CardGroup cols={2}>
  <Card title="Tree-Shakeable API" icon="leaf" href="/typescript/concepts/tree-shakeable-api">
  ...
  <Card title="Error Handling" icon="shield-check" href="/typescript/concepts/error-handling">
