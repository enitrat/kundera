...
import * as crypto from '@kundera-sn/kundera-ts/native';
```
...
```typescript
import { pedersenHash, sign, type Signature } from '@kundera-sn/kundera-ts/native';
```
...
- `@kundera-sn/kundera-ts/native` for FFI-backed crypto
- `@kundera-sn/kundera-ts/wasm` for portable/browser crypto (call `loadWasmCrypto` once)
- `@kundera-sn/kundera-ts/crypto` for auto-selection if you want a single import

All entrypoints share the same API surface.

### isNativeAvailable

Check if native crypto is available (Bun FFI or Node koffi).
...
import * as crypto from '@kundera-sn/kundera-ts/crypto';
...
<Tip>
Native crypto is preferred for performance. For browsers, use the `@kundera-sn/kundera-ts/wasm` entrypoint (or `@kundera-sn/kundera-ts/crypto` if you want auto-selection).
</Tip>
