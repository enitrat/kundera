...
```typescript
// Native (Rust FFI) — synchronous, no loading step
import { pedersen, sign } from '@kundera-sn/kundera-ts/crypto';

const hash = pedersen(a, b);       // works immediately
const signature = sign(pk, msg);
```

```typescript
// WASM — must load async before use
import { loadWasmCrypto } from '@kundera-sn/kundera-ts/wasm';
import { pedersen, sign } from '@kundera-sn/kundera-ts/crypto';

await loadWasmCrypto(); // load once at startup
const hash = pedersen(a, b);       // works after loading
```

<Note>
Both backends export the same crypto API via `@kundera-sn/kundera-ts/crypto`. Native works immediately; WASM requires an async `loadWasmCrypto()` call before any crypto operation.
</Note>
...
`koffi` requires native build tools:
...
| **Node.js backend** | Native | Best performance with koffi |
