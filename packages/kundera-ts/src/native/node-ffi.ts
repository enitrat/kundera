...
function loadLibrary(): NativeFns {
	...
	if (!loadKoffi() || !koffi) {
		throw new Error("koffi not available. Install with: npm install koffi");
	}
	...
	fns = {
		...
		// Hashing
		starknet_pedersen_hash: lib.func(
			`${i32} starknet_pedersen_hash(${ptr} a, ${ptr} b, ${ptr} out)`,
		),
		starknet_poseidon_hash: lib.func(
			`${i32} starknet_poseidon_hash(${ptr} a, ${ptr} b, ${ptr} out)`,
		),
		starknet_poseidon_hash_many: lib.func(
			`${i32} starknet_poseidon_hash_many(${ptr} data, ${u64} count, ${ptr} out)`,
		),
		keccak256: lib.func(
			`${i32} keccak256(${ptr} data, ${u64} len, ${ptr} out)`,
		),
		starknet_keccak256: lib.func(
			`${i32} starknet_keccak256(${ptr} data, ${u64} len, ${ptr} out)`,
		),
		// ECDSA
		starknet_get_public_key: lib.func(
			`${i32} starknet_get_public_key(${ptr} privkey, ${ptr} out)`,
		),
		starknet_sign: lib.func(
			`${i32} starknet_sign(${ptr} privkey, ${ptr} msg, ${ptr} r, ${ptr} s)`,
		),
		starknet_verify: lib.func(
			`${i32} starknet_verify(${ptr} pubkey, ${ptr} msg, ${ptr} r, ${ptr} s)`,
		),
		starknet_recover: lib.func(
			`${i32} starknet_recover(${ptr} msg, ${ptr} r, ${ptr} s, ${ptr} v, ${ptr} out)`,
		),
	};
...
export function pedersenHash(a: Felt252Type, b: Felt252Type): Felt252Type {
	const fns = loadLibrary();
	const out = Buffer.alloc(32);
	const result = fns.starknet_pedersen_hash(
		Buffer.from(a),
		Buffer.from(b),
		out,
	);
...
export function poseidonHash(a: Felt252Type, b: Felt252Type): Felt252Type {
	const fns = loadLibrary();
	const out = Buffer.alloc(32);
	const result = fns.starknet_poseidon_hash(
		Buffer.from(a),
		Buffer.from(b),
		out,
	);
