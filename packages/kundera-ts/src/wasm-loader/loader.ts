...
function resetAllocator(): void {
  if (wasmInstance?.exports.__heap_base) {
    memoryOffset = (wasmInstance.exports.__heap_base as unknown as { value: number }).value;
  } else {
...
function findWasmFile(): string | null {
  ...
  const moduleDir = metaPath ? dirname(metaPath) : dirname(fileURLToPath(import.meta.url));
...
export async function loadWasmCrypto(): Promise<void> {
  ...
  if (!wasmPath) {
    throw new Error('WASM crypto module not found. Build with: cargo build --release --target wasm32-wasip1');
  }
...
export function wasmSign(privateKey: Felt252Type, messageHash: Felt252Type): WasmSignature {
  if (!wasmInstance) throw new Error('WASM not loaded');
  resetAllocator();
  ...
  const result = wasmInstance.exports.starknet_sign(ptrPriv, ptrMsg, ptrR, ptrS);
  checkResult(result);
...
export function wasmVerify(
  ...
  const result = wasmInstance.exports.starknet_verify(ptrPub, ptrMsg, ptrR, ptrS);
...
export function wasmRecover(
  ...
  const result = wasmInstance.exports.starknet_recover(ptrMsg, ptrR, ptrS, ptrV, ptrOut);
  checkResult(result);
