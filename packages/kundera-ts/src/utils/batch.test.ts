...
describe('BatchQueue', () => {
  ...
  describe('clear', () => {
    it('should clear queue and reject pending items', async () => {
      ...
      if (results[0].status === 'rejected') {
        expect((results[0].reason as Error).message).toBe('Queue cleared');
      }
      if (results[1].status === 'rejected') {
        expect((results[1].reason as Error).message).toBe('Queue cleared');
      }
      expect(queue.size()).toBe(0);
    });
  });
  ...
  describe('error handling', () => {
    it('should call onError when batch processing fails', async () => {
      ...
      expect(results[1].status).toBe('rejected');
      if (results[0].status === 'rejected') {
        expect((results[0].reason as Error).message).toBe('Batch processing failed');
      }
      if (results[1].status === 'rejected') {
        expect((results[1].reason as Error).message).toBe('Batch processing failed');
      }
