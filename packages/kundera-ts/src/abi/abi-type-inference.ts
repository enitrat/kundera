					...
					: C extends ","
						? Angles["length"] extends 0
							? Parens["length"] extends 0
								? SplitTopLevel<Rest, "", [...Out, Trim<Current>], Angles, Parens>
...
type MapTypeList<TAbi extends StarknetAbi, TList extends readonly string[]> = {
	[K in keyof TList]: TList[K] extends string ? AbiTypeToTs<TAbi, TList[K]> : never;
};
...
type StructEntries<TAbi extends StarknetAbi> = Extract<
	...
	{ type: "struct" }
>;
type EnumEntries<TAbi extends StarknetAbi> = Extract<TAbi[number], { type: "enum" }>;
...
type LookupEnumEntry<
	TAbi extends StarknetAbi,
	TType extends string,
> = Extract<EnumEntries<TAbi>, { name: TType }>;
...
> = PrimitiveType<Trim<TType>> extends never
	? Trim<TType> extends
			| `core::array::Array<${infer Inner}>`
			| `array::Array<${infer Inner}>`
			| `Array<${infer Inner}>`
		? AbiTypeToTs<TAbi, Inner>[]
		: Trim<TType> extends
				| `core::array::Span<${infer Inner}>`
				| `array::Span<${infer Inner}>`
				| `Span<${infer Inner}>`
			? readonly AbiTypeToTs<TAbi, Inner>[]
			: Trim<TType> extends
					| `core::option::Option<${infer Inner}>`
					| `option::Option<${infer Inner}>`
					| `Option<${infer Inner}>`
				? AbiTypeToTs<TAbi, Inner> | null
...
export type ExtractAbiFunctions<TAbi extends StarknetAbi> = FunctionEntries<TAbi>;
...
> = ExtractAbiFunction<TAbi, TFunctionName>["inputs"] extends readonly []
	? []
	: MapInputs<TAbi, ExtractAbiFunction<TAbi, TFunctionName>["inputs"]> extends
				readonly [infer TSingle]
		? TSingle
...
type EventEntries<TAbi extends StarknetAbi> = Extract<TAbi[number], AbiEventEntry>;
...
export type InferArgs<TAbi, TFunctionName extends string> =
...
export type InferReturn<TAbi, TFunctionName extends string> =
	IsConstAbi<TAbi> extends true
		? TAbi extends StarknetAbi
			? TFunctionName extends ExtractAbiFunctionNames<TAbi>
