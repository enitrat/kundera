...
export function hashMany(values) {
  ...
  let result = /** @type {import('../../primitives/Felt252/types.js').Felt252Type} */ (values[0]);
  for (let i = 1; i < values.length; i++) {
    result = hash(result, /** @type {import('../../primitives/Felt252/types.js').Felt252Type} */ (values[i]));
  }
  return result;
}
