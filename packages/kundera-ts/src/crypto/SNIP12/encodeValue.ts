	...
	return function encodeValue(
		...
		if (type === "selector") {
			if (typeof value !== "string") {
				throw new Snip12EncodingError("selector must be a string", {
					type,
					value,
				});
			}
		...
		if (type === "shortstring") {
			if (typeof value !== "string") {
				throw new Snip12EncodingError("shortstring must be a string", {
					type,
					value,
				});
			}
		...
		if (type === "string" || type === "ByteArray") {
			if (typeof value !== "string") {
				throw new Snip12EncodingError("string must be a string", {
					type,
					value,
				});
			}
 ...
 */
export function createEncodeValue(
	hashStruct: (
		typeName: string,
		data: Record<string, MessageValue>,
		types: TypeDefinitions,
