...
```typescript
import { sign } from 'kundera-sn/wasm';
...
```typescript
import { isWasmAvailable, isWasmLoaded } from 'kundera-sn/wasm';
...
<Note>
Tests require crypto to be available. Test scripts build `wasm/crypto.wasm` before running, and the suite fails if WASM or native crypto can't be loaded.
</Note>
...
| **Memory** | WASM module requires additional memory |
