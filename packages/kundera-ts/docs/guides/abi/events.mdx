...
Kundera provides primitive event decoding in `kundera-sn/abi`. For receipt-wide decoding and filtering, use the event-filtering skill.
...
```typescript
import { decodeEvent, getEventSelector } from 'kundera-sn/abi';
...
```typescript
import { computeSelector } from 'kundera-sn/abi';
import { decodeEvents } from './skills/event-filtering';
