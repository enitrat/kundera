...
describe("presets", () => {
	...
	it.effect("createProvider works with JSON-RPC helpers", () => {
		...
		mockedFetch.mockImplementation(async (_input, init) => {
			...
			const result =
				body.method === "starknet_chainId" ? "0x534e5f5345504f4c4941" : 1;
	...
	it.effect("wallet base stack wires wallet and nonce services", () => {
		...
		mockedFetch.mockImplementation(async (_input, init) => {
			...
			const result =
				body.method === "starknet_chainId" ? "0x534e5f5345504f4c4941" : "0x5";
		...
		return Effect.gen(function* () {
			...
			const blockStream = yield* Services.BlockStreamService;
			const transactionStream = yield* Services.TransactionStreamService;
			...
			expect(typeof blockStream.backfill === "function").toBe(true);
			expect(typeof blockStream.watch === "function").toBe(true);
			expect(typeof transactionStream.watchPending === "function").toBe(true);
			expect(typeof transactionStream.watchConfirmed === "function").toBe(true);
