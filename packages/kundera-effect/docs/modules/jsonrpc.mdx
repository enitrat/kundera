...
<Info>
**Subscriptions vs Streaming** — JSON-RPC subscription methods (`starknet_subscribeNewHeads`, etc.) are not yet wrapped as free functions. However, **event streaming is available today** via [EventService](/effect/services/event). `watchEvents` and `watchDecodedEvents` return `Stream.Stream` and handle polling, deduplication, and backpressure automatically.

See the [Event Service docs](/effect/services/event) for streaming examples.
</Info>
