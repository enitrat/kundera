...
<Info>
**Subscriptions vs Streaming** — JSON-RPC subscription methods (`starknet_subscribeNewHeads`, etc.) are not yet wrapped as free functions. Stream APIs are available through services:

- [BlockStreamService](/effect/services/block-stream)
- [TransactionStreamService](/effect/services/transaction-stream)
- [EventService](/effect/services/event)

These services return `Stream.Stream` and handle polling, deduplication, and retry behavior.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Event Service" icon="bolt" href="/effect/services/event">
    Event streaming with watchEvents and watchDecodedEvents.
  </Card>
  <Card title="Block Stream" icon="layer-group" href="/effect/services/block-stream">
    Stream blocks with backfill and reorg detection.
  </Card>
