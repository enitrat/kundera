...
`TransactionStreamService` provides polling and websocket transaction streams:
...
```typescript
const pendingWs = txStream.watchPending({
  streamMode: "websocket",
  websocket: { url: "wss://api.cartridge.gg/x/starknet/sepolia" },
  filter: { senderAddress: "0x0123..." },
});
```
...
```typescript
const confirmedWs = txStream.watchConfirmed({
  streamMode: "websocket",
  websocket: { url: "wss://api.cartridge.gg/x/starknet/sepolia" },
  confirmations: 3,
  fromBlock: 500_000,
